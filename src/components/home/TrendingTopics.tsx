...
export default async function TrendingTopics() {
  ...
  return (
    <div className="bg-slate-950 border border-slate-800 p-5 sm:p-6 sticky top-24 xl:top-28">
        ...
        {topNews.length > 0 ? (
          topNews.map((news, index) => (
            <Link key={news.id} href={`/berita/${news.slug}`} className="group flex gap-4 items-start pb-4 border-b border-slate-800 last:border-0 last:pb-0">
              <span className="text-4xl font-black text-slate-700 group-hover:text-yellow-400 transition-colors leading-none min-w-[24px] text-center">
                {index + 1}
              </span>
              <div className="flex-1">
                <span className="text-[9px] sm:text-[10px] font-bold text-red-500 block mb-1">
                  {news.categories?.name || 'SPORT'}
                </span>
                <h3 className="text-sm font-black italic group-hover:text-red-500 transition-colors leading-snug line-clamp-2 sm:line-clamp-3 text-slate-200">
                  {news.title}
                </h3>
              </div>
            </Link>
      ...
      <div className="mt-6 sm:mt-8 relative overflow-hidden aspect-video sm:aspect-[4/5] xl:aspect-[4/5] bg-slate-900 text-white flex flex-col items-center justify-center p-6 text-center group cursor-pointer">
        <div className="absolute inset-0 opacity-20 group-hover:opacity-40 transition-opacity bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-yellow-400 to-transparent"></div>
        ...
        <span className="relative z-10 font-bold text-xs uppercase bg-yellow-400 text-slate-900 px-4 py-2 hover:scale-105 active:scale-95 transition-transform delay-75 group-hover:bg-yellow-300">
          Gabung Komunitas
        </span>
      </div>
    </div>
