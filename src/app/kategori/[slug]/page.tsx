...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${categoryData.name} | SkorAkhir`,
    description: categoryData.description || `Kumpulan berita terbaru dan terupdate seputar ${categoryData.name} di SkorAkhir.`,
  };
}
      ...
      <main className="py-8 sm:py-12 flex flex-col container mx-auto px-4 min-h-screen">
        ...
        <div className="mb-10 sm:mb-16 border-b border-slate-800 pb-8 sm:pb-12 text-center sm:text-left">
          <div className="flex flex-col sm:flex-row items-center gap-4 mb-6 justify-center sm:justify-start">
            ...
            <span className="bg-orange-500 text-slate-900 font-black px-4 py-1.5 text-[10px] sm:text-xs uppercase tracking-widest">
              Arsip Berita
            </span>
          </div>
        ...
        {posts.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center bg-slate-900/50 rounded-2xl border border-slate-800/50">
            ...
            <p className="text-slate-400 font-medium">Belum ada artikel yang diterbitkan dalam kategori {categoryData.name}.</p>
