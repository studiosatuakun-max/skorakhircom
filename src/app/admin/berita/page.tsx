  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/berita/create" className="flex items-center gap-2 bg-yellow-400 hover:bg-yellow-500 text-slate-900 px-4 py-2 rounded-lg font-black uppercase text-sm transition-colors">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm font-medium text-slate-300">
              {error && (
                 <tr>
                   <td colSpan={5} className="p-8 text-center text-red-500">{error.message} - Pastikan tabel sudah dibuat.</td>
                 </tr>
              )}
              {news && news.length === 0 && (
                <tr>
              ...
              {news && news.map((item) => (
                <tr key={item.id} className="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="p-4 flex justify-center gap-2">
                    <Link href={`/admin/berita/edit/${item.id}`} className="p-2 bg-slate-800 rounded text-yellow-400 hover:bg-slate-700 transition" aria-label="Edit">
                    ...
                    <form action={async () => {
                      ...
                      <button type="submit" className="p-2 bg-slate-800 rounded text-red-500 hover:bg-slate-700 transition" aria-label="Hapus">
